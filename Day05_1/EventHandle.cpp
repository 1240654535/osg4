...
#include "ListWidget/ModelObject.h"
#include "osgViewer/Viewer"
#include <iostream>
CHandleControl::CHandleControl()
...
bool CHandleControl::handle(const osgGA::GUIEventAdapter& ea,osgGA::GUIActionAdapter& aa)
{
			...
			case osgGA::GUIEventAdapter::LEFT_MOUSE_BUTTON://左键
				{
					...
					if (rpViewer->computeIntersections(ea.getX(),ea.getY(),intersections ) )//viewer的所有节点与屏幕坐标碰撞检测
					{
						...
						//获取选中的物体
						CModelObject* pNode = NULL;
						auto itrNode = nodePath.begin();//遍历节点
						for (;itrNode!=nodePath.end();itrNode++)
						{
							pNode = dynamic_cast<CModelObject*>(*itrNode);//动态转换
...
void CHandleControl::setEventConnect(CEventConnect* pEventConnect)
{
